...
export default function SidebarButton({}) {
  const [nadeToggle, setNadeToggle] = useState(false);
  const [buttonList, setButtonList] = useState(button);

  function handleNadeToggle() {
    setNadeToggle(!nadeToggle);
  }

  function handleToggle() {
    setButtonList(buttonList.map(button => {
      if (button.selected) {
         // Invert the selected state of the button to the copying the array
        return { ...button, selected: false };
      } else {
        return button; // What the fuck?
      }
    }))
  }
  ...
  return nadeToggle ? (
    // Unselected State
    <button className="sidebar-button" onClick={handleNadeToggle}>
      <svg
