...
export default function Split() {
    ...
    function handleClick(index) {
        const nextSidebar = sidebar.map((button, i) => {
          ...
          } else { console.log("Button State Not Found"); return button }
        })
    ...
    // Search Filter
    const search = (nades) => {
        return nades.filter((nade) => nade.title.toLowerCase().includes(query)
    ...
    const results = (nades) => {
        return nades.filter((nade) => 
            sidebar.some(({ type, state, side, tick }) => 
                type === nade.type && (state != false)
            )
        );
    }
    ...
    return (
        <div className="split">
            ...
            <Content results={search(results(Nades))}/>
        </div>
    )
}